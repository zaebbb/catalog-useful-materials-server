import { type ServiceTypePattern } from '@model/NotesModel'
...
export class NoteServicePatternValidator extends BaseValidator<ServiceTypePattern>
  implements Validator<ServiceTypePattern> {
  constructor (values: ServiceTypePattern) {
    super(values)
    this.values = values
  }
  ...
  async apply (): Promise<boolean> {
    ...
    this.setError('linkService', await validate(linkService, {
      isRequired: true,
      isLink: true,
    }))
